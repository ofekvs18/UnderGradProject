"""
ehrshot_evaluate.py — Evaluate method formulas on the EHRSHOT dataset.

Loads master summary CSVs from all four methods and evaluates each formula
on the EHRSHOT modeling data produced by ehrshot_data.py. Uses the EHRSHOT
...
Usage:
    ...
    python src/ehrshot_evaluate.py --disease ra --method m2
"""
...
# ── Formula evaluation (extended: handles infix, GP prefix, and threshold) ────
...
def eval_formula_scores_extended(formula, df, features, bad_frac=0.10):
    ...
    # Standard helpers (also used by methods 2 and 4)
    local["sqrt"] = lambda x: np.sqrt(np.abs(x))
    local["log"]  = lambda x: np.log1p(np.abs(x))
    local["abs"]  = np.abs
    local["neg"]  = lambda x: -x
    # GP primitive functions (method 3 prefix notation)
...
def evaluate_formula_on_ehrshot(formula, train_df, test_df, features):
    """
    Evaluate one formula on the EHRSHOT train/test split.
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate method formulas on the EHRSHOT external validation set"
    )
    ...
    if not data_path.exists():
        sys.exit(
            ...
            f"Run: python src/ehrshot_data.py --ehrshot-dir <path> --disease {args.disease}"
        )
    ...
    print("=" * 70)
    print(f"EHRSHOT Evaluation — {disease.full_name} ({disease.name})")
    print("=" * 70)
    ...
    print(f"EHRSHOT data: {len(df):,} patients | "
    ...
    for method_key in methods_to_run:
        ...
        for formula_col, variant in spec["formulas"]:
            ...
            for _, row in disease_rows.iterrows():
                ...
                result = evaluate_formula_on_ehrshot(formula, train_df, test_df, features)
                if result is None:
                ...
                # Carry through extra context columns from the master summary
