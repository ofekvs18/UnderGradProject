"""
ehrshot_sanity.py — Descriptive statistics for EHRSHOT modeling data.

Reads the EHRSHOT modeling CSV produced by ehrshot_data.py and reports:
  - Dataset overview (N, cases, prevalence, train/test split)
  - Per-feature CBC coverage (fraction of patients with each measurement)
...
Usage:
    ...
    python src/ehrshot_sanity.py --disease t1d
"""
...
def main():
    parser = argparse.ArgumentParser(description="EHRSHOT sanity check and descriptive stats")
    ...
    print("=" * 70)
    print(f"EHRSHOT Sanity Check — {disease.full_name} ({disease.name})")
    print("=" * 70)
    ...
    for feat in features:
        ...
        delta = round(case_mean - ctrl_mean, 4) if not (np.isnan(case_mean) or np.isnan(ctrl_mean)) else np.nan
        comp_rows.append({
            "Timestamp":    timestamp,
            "Disease":      args.disease,
            "Feature":      feat,
    ...
    overview_path = out_dir / "ehrshot_overview.csv"
